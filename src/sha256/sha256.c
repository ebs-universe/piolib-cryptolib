...
static const uint32_t sha256_init_vector[] ROMEM = {
	0x6A09E667UL, 0xBB67AE85UL, 0x3C6EF372UL, 0xA54FF53AUL,
    0x510E527FUL, 0x9B05688CUL, 0x1F83D9ABUL, 0x5BE0CD19UL 
};

#define sha256_init_vector_T_p     ROMEM_TABLE_P(sha256_init_vector)
...
void sha256_init(sha256_ctx_t *state){
    ...
    for (uint8_t i=0; i<8; i++){
        state->h[i] = ROMEM_READ_DWORD(&sha256_init_vector[i]);
    }
}
...
void sha256_nextBlock (sha256_ctx_t *state, const void *block){
    ...
    */
    ...
    for (i=0; i<64; ++i) {
        ...
        t1 = a[7] + SIGMA1(a[4]) + CH(a[4], a[5], a[6]) + ROMEM_READ_DWORD(&k[i]) + w[i % 16];
